...
export async function finalizeCapturedBookingPayment(params: {
  ...
  if (existingPayment) {
    const bd = existingPayment.planPrice !== null && existingPayment.planPrice !== undefined
      ? calculatePaymentBreakdown(existingPayment.planPrice, existingPayment.seatExtraAmount ?? 0)
      : existingPayment.baseAmount !== null && existingPayment.baseAmount !== undefined
        ? calculatePaymentBreakdown(existingPayment.baseAmount)
        : undefined
    return { success: true, alreadyProcessed: true, breakdown: bd, transferId: existingPayment.gatewayTransferId ?? null }
  }
  ...
  if (booking.status === 'CONFIRMED' || booking.status === 'ACTIVE') {
    ...
    const bd = ep?.planPrice !== null && ep?.planPrice !== undefined
      ? calculatePaymentBreakdown(ep.planPrice, ep.seatExtraAmount ?? 0)
      : undefined
  ...
  // ── 5. Recalculate breakdown ───────────────────────────────────────────────
  // Load from PENDING payment record (has planPrice + seatExtraAmount stored at order time)
  const pendingPayment = await prisma.payment.findFirst({ where: { bookingId, status: 'PENDING' } })
  const planPrice      = pendingPayment?.planPrice ?? booking.planPriceSnapshot ?? booking.totalAmount
  const seatExtra      = pendingPayment?.seatExtraAmount ?? booking.seatExtraSnapshot ?? 0
  const breakdown      = calculatePaymentBreakdown(planPrice, seatExtra)
  ...
  if (pendingPayment) {
    ...
    // Fallback: create payment row (webhook path where PENDING record may be missing)
    await prisma.$transaction(async (tx) => {
  ...
  try {
    await prisma.notification.create({
      data: {
        ...
        data: { bookingId, totalAmount: breakdown.totalAmount, ownerAmount: breakdown.ownerAmount, platformFee: breakdown.platformFee, transferId: transferResult.transferId },
      },
    })
  } catch { /* non-critical */ }
