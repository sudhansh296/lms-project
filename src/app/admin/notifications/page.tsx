...
const TYPE_COLOR: Record<string, string> = {
  ...
  PLATFORM_ALERT: 'bg-red-100 text-red-700',
    ...
    <div className="space-y-6">
      ...
      {notifications.length === 0 ? (
        ...
        <div className="space-y-2">
          {notifications.map(n => (
            <div key={n.id}
              className={`rounded-2xl border p-4 transition-colors ${n.isRead ? 'bg-white border-slate-100' : 'bg-indigo-50 border-indigo-100'}`}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 flex-1">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium whitespace-nowrap mt-0.5 ${TYPE_COLOR[n.type] ?? TYPE_COLOR.DEFAULT}`}>
                    {n.type.replace(/_/g, ' ')}
                  </span>
                ...
                {!n.isRead && <div className="w-2 h-2 rounded-full bg-indigo-600 mt-1.5 shrink-0" />}
              </div>
            </div>
          ))}
        </div>
